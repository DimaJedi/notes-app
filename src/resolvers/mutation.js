...
module.exports = {
  newNote: async (parens, args, { models, user }) => {
  ...
  deleteNote: async (parens, { id }, { models, user }) => {
    if (!user) throw new AuthenticationError('You must be signed in to create a note')

    const note = await models.Note.findById(id)
    if (note && String(note.author) !== user.id) throw new ForbiddenError('You dont have permissions to delete the note')
  ...
  updateNote: async (parent, { content, id }, { models }) => {
    ...
    const note = await models.Note.findById(id)
    if (note && String(note.author) !== user.id) throw new ForbiddenError('You dont have permissions to update a note')
